...
export default function FacetOptions(props: any): JSX.Element {
    const {
        index,
        facet,
        onToggle,
    ...
    return (
        ...
        <>
            <div
                ...
                id="select-all" onClick={() => checkAll()}>
                {/* <input type="text" className="!block" id="selectAll"
                    onChange={(e) => { checkAll(e) }}
                    checked={selectAllCheckRef.current}
                /> */}
                {_site?.c_selectAll ? _site?.c_selectAll : t("Select all")}
            </div>
            <div className={cssClasses.optionsContainer}>
                {facetOptions.map((option) => {
                    return <div className={cssClasses.option} key={option.displayName} id={facet.fieldId}>
                        <input
                            type="checkbox"
                        ...
                        />
                        <label className={cssClasses.optionLabel} htmlFor={option.displayName}>
                            {option.displayName}
